...
$p=0;
	...
	if($row['specialite'] == 'psy')
	{$p++;}
	
	if($row['specialite'] == 'pédiatre')
	{$pe++;}
	if($row['specialite'] == 'generaliste')
	{$g++;}
}
  ...
  xAxis: {
	  categories: ['psy','generaliste','pédiatre']
  },
  colors: ['#7673e6', '#f47b25', '#21cb95'],
  series: [{
	  type: 'column',
	  colorByPoint: true,
	  data:[<?php echo json_encode($p)?>, <?php echo json_encode($g)?>,<?php echo json_encode($pe)?>],
	  showInLegend: false
  }]
